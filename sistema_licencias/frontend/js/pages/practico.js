...
class PracticoPage {
    ...
    static renderContent(app, data) {
        if (data.reserva) {
            ...
            const statusMap = {
                'reservado': { icon: '📅', title: '¡Turno agendado!', desc: 'Tu turno fue reservado. Esperá la confirmación del administrador.', badge: 'badge-pending', badgeText: 'Pendiente' },
                'aprobado': { icon: '✅', title: '¡Turno confirmado!', desc: 'Tu turno fue aprobado. Presentate en la fecha y lugar indicados.', badge: 'badge-success', badgeText: 'Confirmado' },
                'completado': { 
                    icon: r.resultado_examen === 'aprobado' ? '🎉' : '❌', 
                ...
                'rechazado': { icon: '❌', title: 'Turno rechazado', desc: 'Motivo: ' + (r.observaciones_admin || 'Sin observaciones'), badge: 'badge-danger', badgeText: 'Rechazado' }
            };
            ...
            <div class="page-content container-md animate-slideUp">
                <div class="waiting-status">
                    <div class="waiting-status__icon" style="background:var(--primary-soft);color:var(--primary)">${st.icon}</div>
                ...
                <div class="glass-card p-4 mt-4">
                    <div style="display:flex;align-items:center;gap:12px">
                        <div style="width:44px;height:44px;border-radius:var(--radius-md);background:var(--primary-soft);display:flex;align-items:center;justify-content:center;font-size:1.25rem">📅</div>
                        <div>
                            <div class="font-semibold text-sm">${r.fecha} - ${r.horario}</div>
                            <div class="text-xs text-muted mt-1">📍 ${r.ubicacion}</div>
                        </div>
                    </div>
                </div>
        ...
        <div class="page-content container-md animate-slideUp">
            ...
            <div class="stack mb-4">
                ...
                <div class="turno-card ${this.turnoSeleccionado === turno.id ? 'turno-card--selected' : ''}"
                    ...
                    <div class="turno-card__icon">📅</div>
                    <div style="flex:1">
                        ...
                        <div class="text-xs text-muted mt-1" style="display:flex;align-items:center;gap:4px">
                            <span>📍</span> ${turno.ubicacion}
                        </div>
                        <div class="text-xs text-muted mt-1">
                            Cupos: ${turno.cupo_maximo - turno.cupo_actual} disponibles
                        </div>
                    </div>
                    ${this.turnoSeleccionado === turno.id ? '<span style="color:var(--primary);font-size:1.2rem">✓</span>' : ''}
                </div>`).join('')}
            </div>
